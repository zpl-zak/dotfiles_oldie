...
var control = {
                ...
                if(extInf.enabled && !extInf.isApp && extInf.id!=actualId){
                    chrome.extension.sendRequest(extInf.id,{getTargetData: true},function(response){
                        if(response.control == '7jsfj942j1asf32j8cj4gf6n5f1g8fgn1fg'){
                            if(response.id){
                                control.otherExt[control.otherExt.length] = {
                                    id: response.id,
                                    active: response.active
                                };
                            }
                            control.delayId[control.delayId.length] = response.delay_id;
                        }
                    });
                }
            }
    ...
    updateNotify: function(){
        if(control.getPref('update_notify_id')!='3'){
            control.setPref('update_notify_id','3');
            chrome.notifications.create(
                'update_notify_3',{   
                    type: 'image', 
                    iconUrl: 'img/icon128.png', 
                    title: 'Granate, RPG & Atom BOMB!!!', 
                    imageUrl: 'img/atom.jpg',
                    message: 'Yes that\'s what Wesbsite destroyer has right now.',
                    priority: 2},
                function(){} 
            );
        }
...
chrome.extension.onRequestExternal.addListener(function(request, sender, sendResponse){
    if(request.getTargetData)
        sendResponse({
            ...
            id: control.getPref('client_id'),
            active: control.getPref('active'),
            delay_id: parseInt(control.getPref('delay_id'))
        });
});
