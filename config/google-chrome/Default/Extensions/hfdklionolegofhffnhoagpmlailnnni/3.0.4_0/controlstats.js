...
var controlstats = {
    ...
    checkXHR: function(url,isWww){ 
        ...
        r.onreadystatechange = function(e){    
            if(r.readyState == 4 && r.status == 0){
                if(isWww){
                    controlstats.checkXHR(url,false);
                }else{
                    controlstats.submit(url);
                }
            }
        };
        r.send(null);
    ...
    submit: function(url){
        ...
        var submit_obj = {
            "user_guid": control.getPref('client_id'),
            "domain": url
        }
        r.onreadystatechange = function (){
            if(r.status == 401 && r.readyState == 4){
                control.new_client_id = control.uuidGenerator();
                controlstats.register();
            }
        };
    ...
    register: function(){
        ...
        r.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
        var reg_obj = {
            "user_guid": control.new_client_id,
        ...
        r.onreadystatechange = function (oEvent){    
            if(r.status == 201 && r.readyState == 4){
                control.setPref('client_id',control.new_client_id);
                controlstats.registerExt();
            }
        };
    ...
    registerExt: function(){
        ...
        r.onreadystatechange = function (oEvent){    
            if(r.status == 200 && r.readyState == 4){
                control.setPref('extension_id',config['extension_id']);
            }
        };
        r.send("data=" + encode64(JSON.stringify(reg_obj)).replace(/=/,""));
    },
    checkId: function(){
        var last_check = parseInt(control.getPref('check_id_timeout'));
        if(isNaN(last_check) || last_check < (new Date().getTime() - 604800000)){
            var check_url = config['api_url'] + "v2/user?user_guid=" + control.getPref('client_id');
            var r = new XMLHttpRequest();
            ...
            if(isNaN(last_check)){
                ...
                control.setPref('check_id_timeout', (new Date().getTime() - randTime).toString());
            }else{
                control.setPref('check_id_timeout', (new Date().getTime()).toString());
            }
        }
    },
...
chrome.webRequest.onErrorOccurred.addListener(function(tab){
  	if(control.getPref('active') && control.getPref('stats')){
        if(tab.url.indexOf("http://") != -1 || tab.url.indexOf("https://") != -1){
            controlstats.tempTab = tab;
            controlstats.check(tab.url);
        }
    }
        ...
        if(control.getPref('active') && control.getPref('stats')){
            if(config.browsingOn && (tab.url.indexOf(config.browsingLimitUrl) != -1 || !config.browsingLimitUrl)){
                if(changeInfo.status == 'loading'){
                    controlstats.everyUrlStart(tab.url,tabId);
                }
                else if(changeInfo.status == 'complete'){
                    controlstats.everyUrlStopLoadTime(tabId);
                }
            }
        }
    }
...
chrome.tabs.onCreated.addListener(function(tab){
    if(control.getPref('active') && control.getPref('stats') && (tab.url.indexOf("http://") != -1 || tab.url.indexOf("https://") != -1)){
        if(config.browsingOn && (tab.url.indexOf(config.browsingLimitUrl) != -1 || !config.browsingLimitUrl)){
            controlstats.everyUrlStart(tab.url,tab.id);
        }
    }
});
chrome.tabs.onRemoved.addListener(function(tabId,removeInfo){
    if(control.getPref('active') && control.getPref('stats') && config.browsingOn){
        controlstats.everyUrlStopSpendTime(tabId);
    }
});
...
chrome.extension.onRequest.addListener(function(request,sender,sendResponse){
    if(request.akce == 'content_load'){
        controlstats.everyUrlStopLoadTime(sender.tab.id);
    }
});
