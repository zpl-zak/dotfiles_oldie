...
var yass = {
  ...
  dump: function(s) {
    if (this.m_pref.dolog) console.log(s); // eslint-disable-line no-console
  },

  pixelsWithCSSDecl: function(decl, name) {
  ...
  handleEvent: function(ev) {
    ...
    var speed = 0.3 * (this.m_sumpixelscroll / this.m_pref.wheelstep / evtime); // distance(ticks) / time
  ...
  ScrollerNoEdge: (function() {
    ...
    F.prototype = {
      bodyClientHeight: function() {
        ...
        return (this.body.clientHeight <= 20 || (this.body.parentNode && this.body.parentNode.clientHeight < this.body.clientHeight))
          ? this.body.parentNode.clientHeight : this.body.clientHeight;
      },
      activate: function() {
        this.etop = yass.edgetop;
        this.ebot = yass.edgebottom;
  ...
  edgetop: {
    ...
    setowner: function(owner) {
      ...
      var s; // local variable for computed style
      this._widthtarget = owner;
      this.e.style.marginBottom = "0px";
      if (owner == owner.ownerDocument.body) {
        s = owner.ownerDocument.defaultView.getComputedStyle(owner, null);
        bodymargintop = yass.pixelsWithCSSDecl(s, "margin-top"); // as pixels
        bodypaddingtop += yass.pixelsWithCSSDecl(s, "padding-top"); // as pixels
        ...
        if (this._ancientnode) {
          ...
          //var origmargin = yass.pixelsWithCSSDecl(owner.ownerDocument.defaultView.getComputedStyle(this._ancientnode, null), "margin-top");
      ...
      for (var i = 0; i < elms.length; i++) {
        ...
        // issue https://code.google.com/p/chromium/issues/detail?id=229280
        // moved to https://bugs.chromium.org/p/chromium/issues/detail?id=589347
        if (s.getPropertyValue("top") == "auto") continue; // ignore elementes with absolute and top:auto style
        if (s.getPropertyValue("position") == "absolute") {
          try { // chrome sometimes throws error here
            var top = yass.pixelsWithCSSDecl(s, "top");
            this.abselms.push([e, top]);
          }
          catch (ex) {}
        }
      }
      // algorithm 2: (new!) search for "absolute" in DOM stylesheets and query with those selectors of rules found
      var regexpAbsolute = /position\s*:\s*absolute/;
      var selectorsAbsolute = [];
      for (var sindex = 0; sindex < document.styleSheets.length; sindex++) {
        var sheet = document.styleSheets[sindex];
      ...
      for (i = 0; i < abselmsCandidate.length; i++) {
        ...
        if (s.getPropertyValue("position") == "absolute") {
          ...
          if (!(this.abselms.find(function(element) { return (element[0] == e); }))) {
            try {
              var top2 = yass.pixelsWithCSSDecl(s, "top");
              this.abselms.push([e, top2]);
            }
            catch (ex) {}
          }
        }
      ...
      this.abselms = this.abselms.filter(function(earg) {
        for (var e = earg[0].parentElement; e != owner && e != null; e = e.parentElement) {
          var cssValue = e.ownerDocument.defaultView.getComputedStyle(e, null).getPropertyValue("position");
          if (cssValue && cssValue != "static") {
            return false;
          }
        }
        return true;
    ...
    adjust: function(ebot) {
      ...
      var w = yass.pixelsWithCSSDecl(ebot.e.ownerDocument.defaultView.getComputedStyle(ebot.e, null), "width");
      if (w == 0) return;
        ...
        if (this.m_resetsmooth > 0) {
          if (this.m_pref.prefversion >= 3) {
            ...
            do { // eslint-disable-line no-constant-condition
              ...
              var b = bdumpfunc(tm - smoothtime + 17 + looptimetotal, bdump);
  ...
  checkDesignMode: function(ev) {
    ...
    var b = true;
    var mode = (ev.target.ownerDocument && ev.target.ownerDocument.designMode) ? ev.target.ownerDocument.designMode : "off";
  ...
  findNodeToScroll: function(orig, hint, log) {
    ...
    var scrollovercheck =
      ...
      (function() {
        ...
        (function(fe, n, a, b) {
          ...
          return (("_yass_ownedby" in n) && n._yass_ownedby) ? n._yass_ownedby.scrollovercheck(hint) : (a < b) && (b > 1); // <- magic code
        })
      );
    ...
    var newobject = function(p) {
      ...
      if (yass.m_edgesize == 0) return yass.ScrollerNoEdge;
    ...
    var frameelement = this.m_frameelement;
    ...
    do {
      ...
      try {
        ...
        if (node.clientWidth && node.clientHeight &&
          ...
          /*log*/
          log += "(" + node.scrollTop + " " + getclientheight(node) + " " + getscrollheight(node) + ")";
  ...
  refreshPreferences: function() {
    chrome.extension.sendMessage({
      ...
      yass.refreshPreferencesCB(res);
    });
  },

  refreshPreferencesCB: function(pref) {
  ...
  messageListener: function(req, send, res) {
...
(function() {
  ...
  chrome.extension.onMessage.addListener(function(req, send, res) {
    yass.messageListener(req, send, res);
  });
})();
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  var i; // local variable for NodeList index
